...
```mermaid
flowchart TB
    subgraph Inner["Inner life"]
        IMP[Impressions]
        APP[Appraisals]
        WANT[Wants & goals]
        BEL[Beliefs]
        THO[Private thoughts]
    end

    subgraph Shared["What you might see"]
        SAY[speech]
        EMO[emotes]
        MAIL[mailbox after dreams]
    end

    Inner -->|sometimes| Shared
    Inner -.->|often stays| Inner
```
...
```mermaid
flowchart LR
    EVENT[Event] --> AP[Appraisal]
    AP --> V[Valence<br/>pleasant ↔ unpleasant]
    AP --> A[Arousal<br/>calm ↔ activated]
    AP --> FL[Feeling label<br/>warm · wary · curious]
    AP --> AT[Action tendency<br/>approach · pause · repair]
    AP --> NOTE["How this lands" note]
```
...
```mermaid
flowchart TB
    SEED[Seed wants & principles] --> MEM[(Memory)]
    EXP[Experience] --> MEM
    MEM --> PLAN[Planning & autonomy]
    PLAN --> REV[Revision expected]
    REV --> MEM
```
...
flowchart TB
    ...
    subgraph Beliefs["Beliefs"]
        B1[What it thinks is true]
        B2[Updated in dreams]
    end

    CH --> Beliefs
    CH --> ST[Self-trust<br/>confidence in judgment]
    CH --> DI[Disposition<br/>default stance]
    CH --> FO[Focus<br/>short-lived working plan]
```
...
flowchart LR
    ...
    C ---|first recognition| B
    Y ---|conversations · sightings| B
    O ---|encounters| B
    Y -.->|may link to| O
```
