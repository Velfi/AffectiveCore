...
pub const EventLog = struct {
    ...
    pub fn append(self: EventLog, kind: []const u8, title: []const u8, body: []const u8) !void {
        try self.appendFn(self.ctx, kind, title, body);
    }

    pub fn setSendEnabled(self: EventLog, enabled: bool) !void {
        const setFn = self.setSendEnabledFn orelse return;
        try setFn(self.ctx, enabled);
    }
};

test "event log interface calls append callback" {
    const State = struct {
        called: bool = false,
    ...
    var state = State{};
    const log = EventLog{ .ctx = &state, .appendFn = State.append };
