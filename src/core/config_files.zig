...
pub fn parseRuntimeOptionsConfig(allocator: std.mem.Allocator, base: Config, bytes: []const u8) !Config {
    ...
    if (value.autonomy_wake_hold_seconds) |v| cfg.autonomy_wake_hold_seconds = v;
    if (value.stimulus_quiescence_seconds) |v| cfg.stimulus_quiescence_seconds = v;
    if (value.stimulus_coalesce_max_wait_seconds) |v| cfg.stimulus_coalesce_max_wait_seconds = v;
...
fn capacityPartialFromFile(file: CapacityConfigFile) CapacityConfigPartial {
    return .{
        ...
        .memory_snippet_max_bytes = file.memory_snippet_max_bytes,
        .memory_context_bytes_max = file.memory_context_bytes_max,
...
pub fn saveRuntimeOptions(allocator: std.mem.Allocator, fs: FileSystem, io: std.Io, cfg: Config) !void {
    const body = try std.json.Stringify.valueAlloc(allocator, .{
        ...
        .autonomy_wake_hold_seconds = cfg.autonomy_wake_hold_seconds,
        .stimulus_quiescence_seconds = cfg.stimulus_quiescence_seconds,
        .stimulus_coalesce_max_wait_seconds = cfg.stimulus_coalesce_max_wait_seconds,
